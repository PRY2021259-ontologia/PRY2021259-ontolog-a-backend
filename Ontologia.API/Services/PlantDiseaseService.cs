...
public class PlantDiseaseService : IPlantDiseaseService
{
    ...
    public PlantDiseaseService(IPlantDiseaseRepository plantDiseaseRepository, IUnitOfWork unitOfWork,
        IUserConceptPlantDiseaseRepository userConceptPlantDiseaseRepository, ISearchService searchService,
        IMapper mapper)
    {
    ...
    public async Task<(PlantDiseaseResponse?, PlantDiseaseByOntologyResource)> GetByOntologyId(string ontologyId)
    {
        try
        {
            ...
            if (existingPlantDisease != null)
                return (new PlantDiseaseResponse(existingPlantDisease), plantDiseaseByOntology);

            if (plantDiseaseByOntology == null)
                return (new PlantDiseaseResponse("PlantDisease Not Found"), new PlantDiseaseByOntologyResource());
        ...
        catch (Exception ex)
        {
            return (new PlantDiseaseResponse($"An error while getting PlantDisease: {ex.Message}"),
                new PlantDiseaseByOntologyResource());
        }
    }
    ...
    public async Task<PlantDiseaseResponse> AssingPlantDiseaseToCategoryDisease(long categoryDiseaseId,
        Guid plantDiseaseId)
    {
        try
        {
            ...
            await _unitOfWork.CompleteAsync();
            var plantDisease = await _plantDiseaseRepository.GetById(plantDiseaseId);
            return new PlantDiseaseResponse(plantDisease);
        }
        catch (Exception ex)
        {
            return new PlantDiseaseResponse(
                $"An error ocurrend while assigning PlantDisease to conceptType: {ex.Message}");
        }
    }

    public async Task<PlantDiseaseResponse> UnassingPlantDiseaseToCategoryDisease(long categoryDiseaseId,
        Guid plantDiseaseId)
    {
        try
        {
        ...
        {
            return new PlantDiseaseResponse(
                $"An error ocurrend while unassigning PlantDisease to conceptType: {ex.Message}");
        }
    }
    ...
    {
        var userConceptsPlantDiseases =
            await _userConceptPlantDiseaseRepository.ListByUserConceptIdAsync(userConceptId);
